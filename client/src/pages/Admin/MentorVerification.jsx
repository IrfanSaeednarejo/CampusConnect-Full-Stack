...
import { getPendingMentorApplications, approveMentorApplication, rejectMentorApplication } from "../../api/adminApi";
...
export default function MentorVerification() {
    ...
    const handleApprove = async (userId) => {
        ...
        try {
            ...
            setPendingMentors((prev) => prev.filter((m) => m._id !== userId));
        } catch (err) { alert("Failed to approve mentor"); }
    ...
    const handleReject = async (userId) => {
        setActionLoading(userId);
        try {
            await rejectMentorApplication(userId, rejectReason);
            setPendingMentors((prev) => prev.filter((m) => m._id !== userId));
            setRejectingId(null);
            setRejectReason("");
        } catch (err) { alert("Failed to reject mentor"); }
        setActionLoading(null);
    };
                                    ...
                                    <div className="flex flex-col sm:flex-row items-start gap-4">
                                        <div className="flex items-center gap-3 flex-1">
                                            {avatar ? (
                                                ...
                                                <div className="w-12 h-12 rounded-full bg-purple-900/30 flex items-center justify-center text-purple-400 font-bold text-lg">
