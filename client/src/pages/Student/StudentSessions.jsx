      ...
      <main className="px-4 sm:px-10 lg:px-20 py-5 md:py-10 max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[#8b949e]">Track your mentoring sessions and feedback.</p>
          </div>
          <button 
            onClick={() => navigate("/student/book-mentor")}
            className="bg-[#238636] hover:bg-[#2ea043] text-white px-5 py-2.5 rounded-lg font-bold text-sm transition-colors flex items-center gap-2"
          >
        ...
        <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-2 mb-8 inline-flex">
          <button
            onClick={() => setActiveTab("upcoming")}
            className={`px-6 py-2 rounded-lg font-medium transition-colors ${
              activeTab === "upcoming" ? "bg-[#238636] text-white" : "text-[#c9d1d9] hover:bg-[#30363d]"
            }`}
          >
          ...
          <button
            ...
            className={`px-6 py-2 rounded-lg font-medium transition-colors ${
              activeTab === "past" ? "bg-[#238636] text-white" : "text-[#c9d1d9] hover:bg-[#30363d]"
            }`}
          >
        ...
        {status === "loading" && (
          <div className="animate-pulse space-y-4">
            <div className="h-40 bg-[#161b22] rounded-xl border border-[#30363d]"></div>
            <div className="h-40 bg-[#161b22] rounded-xl border border-[#30363d]"></div>
          </div>
        )}
        ...
        {status === "succeeded" && displaySessions.length === 0 ? (
          <div className="bg-[#161b22] border border-[#30363d] rounded-xl p-16 flex flex-col items-center justify-center text-center">
            <span className="material-symbols-outlined text-6xl text-[#8b949e] mb-4">forum</span>
            <h3 className="text-xl font-bold text-white mb-2">No {activeTab} sessions</h3>
            <p className="text-[#8b949e] mb-6">Book a session to get personalized guidance.</p>
            <button 
              onClick={() => navigate("/student/book-mentor")}
              className="border border-[#30363d] text-[#c9d1d9] hover:bg-[#30363d] px-6 py-2 rounded-lg font-bold text-sm transition-colors"
            >
              Find a Mentor
            </button>
          </div>
        ...
        <div>
          <h3 className="text-white text-lg font-bold leading-tight flex items-center gap-2 mb-1">
            ...
            {activeTab === 'past' && session.feedbackGiven && (
              <span className="bg-[#238636]/20 text-[#238636] border border-[#238636] text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider">
                Feedback Given ✓
              </span>
            )}
          </h3>
          <p className="text-[#8b949e] text-sm mb-2">{session.mentorTitle} at {session.mentorCompany}</p>
          <div className="flex flex-wrap gap-4 text-sm text-[#c9d1d9]">
            <span className="flex items-center gap-1.5"><span className="material-symbols-outlined text-[16px] text-[#8b949e]">calendar_today</span>{session.date}</span>
            <span className="flex items-center gap-1.5"><span className="material-symbols-outlined text-[16px] text-[#8b949e]">schedule</span>{session.time}</span>
            <span className="flex items-center gap-1.5"><span className="material-symbols-outlined text-[16px] text-[#8b949e]">timer</span>{session.duration}</span>
            <span className="flex items-center gap-1.5"><span className="material-symbols-outlined text-[16px] text-[#8b949e]">topic</span>{session.topic}</span>
          </div>
        </div>
      </div>
          ...
          <>
            {!session.feedbackGiven && (
              <button
                ...
                disabled={isLoading}
                className="w-full justify-center px-4 py-2 rounded-lg border border-[#238636] text-[#2ea043] hover:bg-[#238636]/10 font-bold text-sm transition-colors flex items-center gap-1.5"
              >
