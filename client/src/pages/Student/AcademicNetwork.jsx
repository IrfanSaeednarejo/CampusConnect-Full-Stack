...
export default function AcademicNetwork() {
  ...
  const roleColors = {
    ...
    'Student': 'bg-green-500/20 text-green-400 border-primary/30',
      ...
      <main className="flex-1 overflow-y-auto w-full custom-scrollbar">
        <div className="p-4 sm:p-8 lg:p-12 max-w-7xl mx-auto space-y-8">
          ...
          <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border pb-6">
            <div>
              <h1 className="text-3xl font-extrabold text-text-primary tracking-tight">Academic Network</h1>
          ...
          <nav className="flex flex-wrap gap-2 text-sm font-medium">
            <button onClick={() => navigate('/student/events')} className="px-4 py-2 border border-border text-text-secondary hover:border-primary hover:text-green-400 rounded-full text-sm font-medium transition-colors duration-200">Career Events</button>
            <button onClick={() => navigate('/student/book-mentor')} className="px-4 py-2 border border-border text-text-secondary hover:border-primary hover:text-green-400 rounded-full text-sm font-medium transition-colors duration-200">Find a Mentor</button>
            <button onClick={() => navigate('/student/societies')} className="px-4 py-2 border border-border text-text-secondary hover:border-primary hover:text-green-400 rounded-full text-sm font-medium transition-colors duration-200">Explore Societies</button>
          </nav>
          ...
          <section id="directory" className="pt-4" ref={gridTopRef}>
            <div className="bg-surface border border-border rounded-xl p-6 shadow-sm mb-8">
              ...
              <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div className="relative w-full md:w-1/3">
                  ...
                  <input
                    ...
                    className="w-full bg-background border border-border text-text-primary pl-10 pr-10 py-2.5 rounded-lg focus:outline-none focus:border-blue-500 transition-colors"
                  />
                  {searchTerm && (
                    <button 
                      onClick={() => {
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <select 
                  ...
                  className="bg-background border border-border text-text-primary py-2 px-3 rounded-lg focus:outline-none focus:border-blue-500 transition-colors appearance-none"
                >
              ...
              <div className="mt-6 flex justify-start items-center gap-3">
                ...
                <select 
                  ...
                  className="bg-background border border-border text-text-primary py-1.5 px-3 rounded-lg text-sm focus:outline-none focus:border-blue-500 transition-colors appearance-none"
                >
            ...
            {status === 'loading' ? (
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        <div className="relative">
                          <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold border-2 ${
                            ...
                            profile.role === 'Student' ? 'bg-green-900 border-primary text-green-200' :
                            profile.role === 'Alumni' ? 'bg-orange-900 border-orange-500 text-orange-200' :
                            'bg-purple-900 border-purple-500 text-purple-200'
                          }`}>
                            {getInitials(profile.name)}
                          </div>
                          <span className={`absolute bottom-0 right-0 w-4 h-4 rounded-full border-2 border-background ${profile.isOnline ? 'bg-green-500' : 'bg-[#475569]'}`}></span>
                        </div>
                        <div className="flex-1">
                          <h3 className="text-lg font-bold text-text-primary leading-tight">{profile.name}</h3>
                          <p className="text-sm text-text-secondary mt-1">{profile.department}</p>
                          <span className={`inline-block mt-2 px-2 py-0.5 rounded text-xs font-bold border ${roleColors[profile.role] || 'bg-surface border border-border border-border text-text-secondary'}`}>
                        ...
                        {profile.connectionStatus === 'connected' && (
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 bg-surface-hover hover:bg-[#C7D2FE] text-text-primary border border-border font-bold py-2 rounded-lg flex justify-center items-center h-10 transition-colors text-sm"
                            >
                              Profile
                            </button>
                            <button
                ...
                {totalPages > 1 && (
                  <div className="flex justify-center items-center gap-2 mt-8">
                    <button
                      ...
                      className="w-10 h-10 rounded-lg border border-border flex items-center justify-center text-text-secondary hover:text-text-primary hover:bg-surface-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                      <button
                        ...
                        className={`w-10 h-10 rounded-lg border font-medium flex items-center justify-center transition-colors ${
                          currentPage === page
                            ? 'bg-primary border-[#4338CA] text-white'
                            : 'bg-background border-border text-text-secondary hover:text-text-primary hover:bg-surface-hover'
                        }`}
                      >
                        {page}
                      </button>
              ...
              <div className="flex flex-col items-center justify-center py-24 px-6 text-center border border-border rounded-xl bg-surface">
                ...
                <h3 className="text-xl font-bold text-text-primary mb-2">No matching profiles found</h3>
                ...
                <button
                  ...
                  className="mt-6 px-6 py-2 bg-surface-hover border border-border rounded-lg text-text-primary hover:bg-[#C7D2FE] transition-colors font-medium"
                >
                  Clear Filters
                </button>
              </div>
