      ...
      <main className="px-4 sm:px-10 lg:px-20 py-5 md:py-10 max-w-6xl mx-auto">
        ...
        {status === "loading" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="animate-pulse h-64 bg-surface rounded-xl border border-border"></div>
            ))}
          </div>
        )}
              ...
              <div key={mentor._id} className="bg-surface border border-border rounded-xl p-6 flex flex-col hover:border-primary/50 hover:shadow-lg hover:shadow-[#238636]/5 transition-all duration-300 group">
                <div className="flex gap-4 mb-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-text-secondary text-xs">({mentor.totalSessions} sessions)</span>
                    </div>
                  </div>
                </div>

                {mentor.bio && (
                  <p className="text-text-secondary text-sm mb-4 line-clamp-2">{mentor.bio}</p>
                )}
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  {mentor.expertise.length > 4 && (
                    <span className="text-text-secondary text-xs px-2 py-1">+{mentor.expertise.length - 4} more</span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-4 border-t border-[#21262d]">
                  ...
                  <button
                    ...
                    className="bg-primary hover:bg-primary-hover text-white py-2 px-5 rounded-lg font-bold text-sm transition-colors flex items-center gap-2"
                  >
