...
function EventActionBlock({ event }) {
  ...
  if (liveStatus === 'upcoming') {
    if (!isRegistered) {
      ...
      icon = "add_circle";
      activeClasses = "bg-[#238636] text-[#0d1117] hover:bg-[#2ea043]";
      ...
      icon = "check_circle";
      activeClasses = "bg-[#161b22] text-[#238636] border border-[#238636] hover:bg-[#238636]/10";
    ...
    if (isRegistered) {
      ...
      icon = "do_not_disturb";
      disabled = true;
      activeClasses = "bg-[#30363d] text-[#8b949e] cursor-not-allowed";
    }
  } else if (liveStatus === 'completed') {
    if (isRegistered) {
      buttonText = "View Details";
      icon = "info";
      activeClasses = "bg-[#161b22] text-[#c9d1d9] border border-[#30363d] hover:bg-[#30363d]";
      ...
      <main className="px-4 sm:px-10 lg:px-20 py-5 md:py-10 max-w-6xl mx-auto">
        ...
        {status === 'loading' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="animate-pulse bg-[#161b22] h-72 rounded-lg border border-[#30363d]"></div>
            ))}
          </div>
        )}
        ...
        {status === 'succeeded' && displayEvents.length === 0 ? (
          <div className="bg-[#161b22] border border-[#30363d] rounded-lg p-16 flex flex-col items-center justify-center text-center">
            <span className="material-symbols-outlined text-6xl text-[#8b949e] mb-4">
              event_busy
            </span>
            <h3 className="text-xl font-bold text-white mb-2">No {activeTab} events yet</h3>
            <p className="text-[#8b949e]">Check back later for new opportunities.</p>
          </div>
        ) : status === 'succeeded' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayEvents.map((event) => (
              ...
              >
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold border ${
                        event.liveStatus === "ongoing"
                          ...
                          : event.isRegistered
                          ? "bg-[#238636]/10 text-[#238636] border-[#238636]"
                ...
                <div className="p-5 flex flex-col flex-1 gap-4">
                  ...
                  <p className="text-[#8b949e] text-sm line-clamp-2 min-h-[40px]">
                    {event.description}
                  </p>

                  {/* Event Details */}
                  <div className="space-y-2 py-3 mt-auto border-t border-b border-[#30363d]">
                    ...
                    <div className="flex items-center gap-2 text-[#c9d1d9] text-sm">
                      <span className="material-symbols-outlined text-[#8b949e]">location_on</span>
