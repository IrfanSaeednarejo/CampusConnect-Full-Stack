...
export default function RequestAcceptedConfirmation() {
  ...
  return (
    ...
    >
      ...
      {/* Modal Container */}
      <div className="relative flex w-full max-w-md flex-col items-center gap-4 rounded-lg border border-border bg-surface p-6 text-center text-[#e6edf3] shadow-2xl">
        {/* Close Button */}
        <button
          onClick={() => navigate(-1)}
          className="absolute top-4 right-4 text-text-secondary transition-colors hover:text-[#e6edf3]"
        >
        ...
        <div className="flex w-full flex-col items-stretch gap-3 pt-4">
          ...
          <button
            onClick={() => navigate(-1)}
            className="flex h-11 cursor-pointer items-center justify-center overflow-hidden rounded-lg border border-border bg-surface-hover px-5 text-sm font-semibold text-[#e6edf3] transition-colors hover:border-[#8b949e]"
          >
