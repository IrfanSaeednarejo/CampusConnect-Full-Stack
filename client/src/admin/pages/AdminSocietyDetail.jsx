    ...
    return (
        <div style={{ animation: "fadeIn 0.5s ease-out" }}>
            <div style={{ marginBottom: 32, display: "flex", alignItems: "center", gap: 16 }}>
                <Link to="/admin/societies" className={getButtonClassName({ variant: "ghost", size: "sm", isDark, className: "min-w-0 px-2 no-underline" })}>
                    ← BACK TO SOCIETIES
                </Link>
            </div>
                    ...
                    <div style={{ background: "#0f172a", borderRadius: 20, padding: 24, border: "1px solid #1e293b" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            {society.status === "pending" && (
                                <>
                                    <Button variant="success" size="sm" className="w-full justify-center" onClick={() => setStatusModal({ type: "approved" })}>✓ APPROVE SOCIETY</Button>
                                    <Button variant="danger" size="sm" className="w-full justify-center" onClick={() => setStatusModal({ type: "rejected" })}>✕ REJECT SOCIETY</Button>
                                </>
                            )}
                            {society.status === "approved" && (
                                <Button variant="warning" size="sm" className="w-full justify-center" onClick={() => setStatusModal({ type: "archived" })}>FREEZE SOCIETY</Button>
                            )}
                            {society.status === "archived" && (
                                <Button variant="success" size="sm" className="w-full justify-center" onClick={() => setStatusModal({ type: "approved" })}>REACTIVATE SOCIETY</Button>
                            )}
                            <div style={{ height: 1, background: "#1e293b", margin: "8px 0" }} />
                            <Button variant="danger" size="sm" className="w-full justify-center" onClick={() => setDeleteModalOpen(true)}>PERMANENTLY DELETE</Button>
                        </div>
                    </div>
